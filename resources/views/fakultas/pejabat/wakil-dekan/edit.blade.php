...
@section('title', 'Edit Wakil Dekan')
@push('styles')
<style>
...
@endpush
@section('content')
    <div class="page-header"><h1><i class="fas fa-user-edit" style="margin-right:10px"></i> Edit Wakil Dekan</h1></div>
    <div class="form-card">
        <div class="form-card-header">
            <div class="form-card-header-icon"><i class="fas fa-users"></i></div>
            <h3>Edit Data Wakil Dekan</h3>
        </div>
        <form method="POST" action="{{ route('fakultas.wakil-dekan.update', $wakilDekan->id) }}">
            @csrf @method('PUT')
            <div class="form-card-body">
                <div class="form-group"><label><i class="fas fa-user"></i> Nama</label><input type="text" name="nama" class="form-input" value="{{ old('nama', $wakilDekan->nama) }}" required></div>
                <div class="form-group"><label><i class="fas fa-id-card"></i> NIP</label><input type="text" name="nip" class="form-input" value="{{ old('nip', $wakilDekan->nip) }}"></div>
                <div class="form-group"><label><i class="fas fa-briefcase"></i> Bidang</label><input type="text" name="bidang" class="form-input" value="{{ old('bidang', $wakilDekan->bidang) }}"></div>
                <div class="form-group"><label><i class="fas fa-calendar"></i> Periode Mulai</label><input type="date" name="periode_mulai" class="form-input" value="{{ old('periode_mulai', $wakilDekan->periode_mulai) }}"></div>
                <div class="form-group"><label><i class="fas fa-calendar-check"></i> Periode Selesai</label><input type="date" name="periode_selesai" class="form-input" value="{{ old('periode_selesai', $wakilDekan->periode_selesai) }}"></div>
            </div>
