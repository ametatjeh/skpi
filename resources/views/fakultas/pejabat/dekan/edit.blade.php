...
@endpush
@section('content')
    <div class="page-header"><h1><i class="fas fa-user-edit" style="margin-right:10px"></i> Edit Dekan</h1></div>
    <div class="form-card">
        <div class="form-card-header">
            <div class="form-card-header-icon"><i class="fas fa-user-tie"></i></div>
            <h3>Edit Data Dekan</h3>
        </div>
        <form method="POST" action="{{ route('fakultas.dekan.update', $dekan->id) }}">
            @csrf @method('PUT')
            <div class="form-card-body">
                <div class="form-group"><label><i class="fas fa-user"></i> Nama</label><input type="text" name="nama" class="form-input" value="{{ old('nama', $dekan->nama) }}" required></div>
                <div class="form-group"><label><i class="fas fa-id-card"></i> NIP</label><input type="text" name="nip" class="form-input" value="{{ old('nip', $dekan->nip) }}"></div>
                <div class="form-group"><label><i class="fas fa-calendar"></i> Periode Mulai</label><input type="date" name="periode_mulai" class="form-input" value="{{ old('periode_mulai', $dekan->periode_mulai) }}"></div>
                <div class="form-group"><label><i class="fas fa-calendar-check"></i> Periode Selesai</label><input type="date" name="periode_selesai" class="form-input" value="{{ old('periode_selesai', $dekan->periode_selesai) }}"></div>
            </div>
