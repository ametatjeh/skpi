...
<style>
    ...
    .form-card { background:#fff; border-radius:16px; border:1px solid #e5e7eb; box-shadow:0 4px 16px rgba(0,0,0,0.04); overflow:hidden; max-width:700px; }
    ...
    .form-footer { padding:16px 24px; border-top:1px solid #e5e7eb; display:flex; justify-content:flex-end; gap:10px; background:#f8fafc; }
...
@section('content')
    <div class="page-header">
        <h1><i class="fas fa-university" style="margin-right:10px"></i> Edit Data Fakultas</h1>
        <p>Kelola informasi umum fakultas Anda</p>
    </div>
        ...
        <form method="POST" action="{{ route('fakultas.profile.update') }}">
            ...
            <div class="form-card-body">
                <div class="form-group">
                    <label><i class="fas fa-university"></i> Nama Fakultas</label>
                    <input type="text" name="nama_fakultas" class="form-input" value="{{ old('nama_fakultas', $fakultas->nama_fakultas ?? '') }}" required>
                </div>
                <div class="form-group">
                    <label><i class="fas fa-user-tie"></i> Nama Dekan</label>
                    <input type="text" name="dekan" class="form-input" value="{{ old('dekan', $fakultas->dekan ?? '') }}">
                </div>
                <div class="form-group">
                    <label><i class="fas fa-award"></i> Akreditasi</label>
                    <input type="text" name="akreditasi" class="form-input" value="{{ old('akreditasi', $fakultas->akreditasi ?? '') }}" placeholder="A / B / C / Unggul">
                </div>
                <div class="form-group">
                    <label><i class="fas fa-file-contract"></i> Nomor SK</label>
                    <input type="text" name="no_sk" class="form-input" value="{{ old('no_sk', $fakultas->no_sk ?? '') }}">
                </div>
            </div>
