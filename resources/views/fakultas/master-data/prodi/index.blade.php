...
<style>
    ...
    .card-header { padding:18px 24px; border-bottom:1px solid #f1f5f9; display:flex; align-items:center; justify-content:space-between; background:linear-gradient(135deg,#faf5ff,#f3e8ff); }
    ...
    .prodi-badge { padding:4px 10px; background:#f3e8ff; color:#7c3aed; border-radius:6px; font-size:11px; font-weight:600; }
</style>
@endpush
@section('content')
    <div class="page-header">
        <h1><i class="fas fa-building" style="margin-right:10px"></i> Program Studi</h1>
        <p>Daftar program studi di {{ $fakultas->nama_fakultas ?? 'Fakultas' }}</p>
    </div>
    <div class="data-card">
        <div class="card-header">
            ...
            <span class="prodi-badge">{{ count($prodis ?? []) }} Prodi</span>
        </div>
        <div style="overflow-x:auto">
            <table class="modern-table">
                <thead>
                    <tr><th>#</th><th>Nama Program Studi</th><th>Jenjang</th><th>Akreditasi</th></tr>
                </thead>
                <tbody>
                    ...
                    @empty
                        <tr><td colspan="4" style="text-align:center;padding:40px;color:#9ca3af">Belum ada prodi</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
