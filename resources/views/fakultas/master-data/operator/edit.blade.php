...
@section('content')
    <div class="page-header">
        <h1><i class="fas fa-user-edit" style="margin-right:10px"></i> Edit Operator</h1>
    </div>
    <div class="form-card">
        <div class="form-card-header">
            <div class="form-card-header-icon"><i class="fas fa-user-edit"></i></div>
            <h3>Edit Data Operator</h3>
        </div>
        <form method="POST" action="{{ route('fakultas.operators.update', $operator->id ?? 0) }}">
            @csrf @method('PUT')
            <div class="form-card-body">
                <div class="form-group">
                    <label><i class="fas fa-user"></i> Nama</label>
                    <input type="text" name="name" class="form-input" value="{{ old('name', $operator->name ?? '') }}" required>
                </div>
                <div class="form-group">
                    <label><i class="fas fa-envelope"></i> Email</label>
                    <input type="email" name="email" class="form-input" value="{{ old('email', $operator->email ?? '') }}" required>
                </div>
                <div class="form-group">
                    <label><i class="fas fa-lock"></i> Password Baru <small style="color:#9ca3af">(kosongkan jika tidak ingin ubah)</small></label>
