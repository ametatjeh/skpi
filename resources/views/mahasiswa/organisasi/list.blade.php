...
<style>
    ...
    .item-icon { width: 42px; height: 42px; border-radius: 10px; background: linear-gradient(135deg, #dcfce7 0%, #bbf7d0 100%); color: #10b981; display: flex; align-items: center; justify-content: center; font-size: 18px; flex-shrink: 0; }
...
<div class="achievement-container">
    <div class="achievement-header">
        <div class="achievement-header-left">
            <div class="achievement-header-icon"><i class="fas fa-users"></i></div>
            <div>
                <h1>Organisasi</h1>
                <p>Kelola pengalaman organisasi dan kepemimpinan Anda</p>
            </div>
        </div>
        <a href="{{ route('mahasiswa.organisasi.create') }}" class="btn-add-new">
            <i class="fas fa-plus-circle"></i> Tambah Organisasi
        </a>
    </div>
    ...
    <div class="stats-grid">
        <div class="stat-card draft">
            ...
            <div class="stat-value">{{ $organisasi->where('status', 'draft')->count() }}</div>
            <div class="stat-label">Draft</div>
        </div>
        <div class="stat-card pending">
            <div class="stat-icon"><i class="fas fa-clock"></i></div>
            <div class="stat-value">{{ $organisasi->where('status', 'submitted')->count() }}</div>
            <div class="stat-label">Menunggu</div>
        </div>
        <div class="stat-card approved">
            <div class="stat-icon"><i class="fas fa-check-circle"></i></div>
            <div class="stat-value">{{ $organisasi->where('status', 'approved')->count() }}</div>
            <div class="stat-label">Disetujui</div>
        </div>
        <div class="stat-card rejected">
            <div class="stat-icon"><i class="fas fa-times-circle"></i></div>
            <div class="stat-value">{{ $organisasi->whereIn('status', ['rejected', 'revision_required'])->count() }}</div>
    ...
    <div class="table-card">
        <div class="table-header">
            <div class="table-title"><i class="fas fa-list"></i> Daftar Organisasi</div>
            <span class="table-badge">{{ $organisasi->count() }} Data</span>
        </div>
                                ...
                                <td>
                                    <div class="item-info">
                                        ...
                                        <div class="item-detail">
                                            <h4>{{ $item->nama_organisasi }}</h4>
                                            <p>{{ $item->posisi }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td>{{ $item->tahun_masuk }} - {{ $item->tahun_keluar ?? 'Sekarang' }}</td>
                                <td>
                                ...
                                <td>
                                    <div class="action-buttons">
                                        @if ($item->status == 'draft')
                                            <a href="{{ route('mahasiswa.organisasi.edit', $item->id) }}" class="btn-action btn-edit"><i class="fas fa-edit"></i> Edit</a>
                                            <form action="{{ route('mahasiswa.organisasi.submit', $item->id) }}" method="POST" style="margin:0;">@csrf<button type="submit" class="btn-action btn-submit"><i class="fas fa-paper-plane"></i> Submit</button></form>
                                            <form action="{{ route('mahasiswa.organisasi.destroy', $item->id) }}" method="POST" style="margin:0;">@csrf @method('DELETE')<button type="submit" class="btn-action btn-delete"><i class="fas fa-trash"></i></button></form>
                                        @elseif($statusVerifikasi == 'revision_required')
                                            <a href="{{ route('mahasiswa.organisasi.edit', $item->id) }}" class="btn-action btn-revision"><i class="fas fa-edit"></i> Revisi</a>
                                        @elseif($statusVerifikasi == 'pending')
                                            <span class="btn-action btn-waiting"><i class="fas fa-clock"></i> Menunggu</span>
                                        @else
                                            <a href="{{ route('mahasiswa.organisasi.show', $item->id) }}" class="btn-action btn-detail"><i class="fas fa-eye"></i> Detail</a>
                                        @endif
                                    </div>
                                </td>
                            </tr>
        ...
        @else
            <div class="empty-state">
                ...
                <p>Mulai dengan menambahkan pengalaman organisasi Anda</p>
                <a href="{{ route('mahasiswa.organisasi.create') }}" class="btn-add-new"><i class="fas fa-plus-circle"></i> Tambah Organisasi</a>
            </div>
        @endif
    </div>
</div>
