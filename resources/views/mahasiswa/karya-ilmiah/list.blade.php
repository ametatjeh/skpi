...
@section('title', 'Daftar Karya Ilmiah')
@section('page_title', 'Karya Ilmiah')
@section('page_icon', 'book')
...
@section('content')
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px;">
        <div>
            <h2 style="font-size: 24px; font-weight: 800; margin: 0;">Karya Ilmiah</h2>
            <p style="color: var(--muted); margin: 8px 0 0 0;">Kelola semua karya ilmiah Anda</p>
        </div>
        <a href="{{ route('mahasiswa.karya.create') }}" class="btn btn-primary" style="padding: 10px 20px;">
            <i class="fas fa-plus"></i> Tambah Karya Ilmiah
        </a>
    </div>
    ...
    <div
        ...
        <div class="card"
            ...
            <div style="font-size: 32px; font-weight: 700;">{{ $karya_ilmiah->where('status', 'draft')->count() }}</div>
        ...
        <div class="card"
            ...
            <div style="font-size: 32px; font-weight: 700;">{{ $karya_ilmiah->where('status', 'submitted')->count() }}</div>
        ...
        <div class="card"
            ...
            <div style="font-size: 32px; font-weight: 700;">{{ $karya_ilmiah->where('status', 'approved')->count() }}</div>
        ...
        <div class="card"
            ...
            <div style="font-size: 32px; font-weight: 700;">
                {{ $karya_ilmiah->whereIn('status', ['rejected', 'revision_required'])->count() }}</div>
    ...
    <div class="card">
        @if ($karya_ilmiah->count() > 0)
            <div class="table-container">
                <table class="table">
                    ...
                    <tbody>
                        @foreach ($karya_ilmiah as $key => $item)
                            <tr>
                                <td style="padding: 16px;">{{ $key + 1 }}</td>
                                <td style="padding: 16px;">
                                    <strong>{{ $item->judul_karya }}</strong>
                                    @if ($item->doi_issn)
                                        <br><small style="color: var(--muted);">{{ $item->doi_issn }}</small>
                                    @endif
                                </td>
                                <td style="padding: 16px; text-transform: capitalize;">
                                    {{ $item->jenis_publikasi }}
                                </td>
                                <td style="padding: 16px;">{{ $item->nama_jurnal_konferensi }}</td>
                                <td style="padding: 16px;">{{ $item->tahun_publikasi }}</td>
                                <td style="padding: 16px;">
                                ...
                                <td style="padding: 16px;">
                                    <div style="display: flex; gap: 8px; flex-wrap: wrap;">
                                        @if ($item->status == 'draft')
                                            <!-- Edit Button -->
                                            <a href="{{ route('mahasiswa.karya.edit', $item->id) }}" class="btn btn-sm"
                                                style="background: #3b82f6; color: white; padding: 6px 12px; border-radius: 6px; text-decoration: none; font-size: 12px;">
                                                <i class="fas fa-edit"></i> Edit
                                            </a>

                                            <!-- Submit Button -->
                                            <form action="{{ route('mahasiswa.karya.submit', $item->id) }}" method="POST"
                                            ...
                                            <!-- Delete Button -->
                                            <form action="{{ route('mahasiswa.karya.destroy', $item->id) }}" method="POST"
                                                style="display: inline;">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-sm"
                                        ...
                                        @else
                                            <!-- View Button -->
                                            <a href="{{ route('mahasiswa.karya.show', $item->id) }}" class="btn btn-sm"
        ...
        @else
            <div style="padding: 60px 24px; text-align: center;">
                ...
                <p style="color: var(--muted); font-size: 14px; margin-bottom: 24px;">Mulai dengan menambahkan karya ilmiah
                    baru</p>
                <a href="{{ route('mahasiswa.karya.create') }}" class="btn btn-primary" style="padding: 10px 20px;">
                    <i class="fas fa-plus"></i> Tambah Karya Ilmiah
                </a>
