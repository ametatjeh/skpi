...
@section('page_title', 'Sertifikasi Kompetensi')
...
<div class="achievement-container">
    <div class="achievement-header">
        <div class="achievement-header-left">
            <div class="achievement-header-icon"><i class="fas fa-certificate"></i></div>
            <div>
                <h1>Sertifikasi Kompetensi</h1>
                <p>Kelola sertifikat keahlian profesional Anda</p>
            </div>
        </div>
        <a href="{{ route('mahasiswa.sertifikasi.create') }}" class="btn-add-new">
    ...
    @if ($message = Session::get('info'))
        <div class="alert-premium info"><i class="fas fa-info-circle"></i> {{ $message }}</div>
    @endif

    <div class="stats-grid">
        <div class="stat-card draft">
            <div class="stat-icon"><i class="fas fa-file-alt"></i></div>
            <div class="stat-value">{{ $sertifikasi->where('status', 'draft')->count() }}</div>
            <div class="stat-label">Draft (Belum Submit)</div>
        </div>
        <div class="stat-card pending">
            <div class="stat-icon"><i class="fas fa-clock"></i></div>
            <div class="stat-value">{{ $sertifikasi->where('status', 'submitted')->count() }}</div>
            <div class="stat-label">Menunggu Verifikasi</div>
        </div>
        <div class="stat-card approved">
            <div class="stat-icon"><i class="fas fa-check-circle"></i></div>
            <div class="stat-value">{{ $sertifikasi->where('status', 'approved')->count() }}</div>
            <div class="stat-label">Disetujui</div>
        </div>
        <div class="stat-card rejected">
            <div class="stat-icon"><i class="fas fa-times-circle"></i></div>
            <div class="stat-value">{{ $sertifikasi->whereIn('status', ['rejected', 'revision_required'])->count() }}</div>
    ...
    <div class="table-card">
        <div class="table-header">
            <div class="table-title"><i class="fas fa-list"></i> Daftar Sertifikasi</div>
            <span class="table-badge">{{ $sertifikasi->count() }} Data</span>
        </div>
                                ...
                                <td>
                                    <div class="action-buttons">
                                        @if ($item->status == 'draft')
                                            <a href="{{ route('mahasiswa.sertifikasi.edit', $item->id) }}" class="btn-action btn-edit"><i class="fas fa-edit"></i> Edit</a>
                                            <form action="{{ route('mahasiswa.sertifikasi.submit', $item->id) }}" method="POST" style="margin:0;">@csrf<button type="submit" class="btn-action btn-submit"><i class="fas fa-paper-plane"></i> Submit</button></form>
                                            <form action="{{ route('mahasiswa.sertifikasi.destroy', $item->id) }}" method="POST" style="margin:0;">@csrf @method('DELETE')<button type="submit" class="btn-action btn-delete"><i class="fas fa-trash"></i></button></form>
                                        @elseif($statusVerifikasi == 'revision_required')
                                            <a href="{{ route('mahasiswa.sertifikasi.edit', $item->id) }}" class="btn-action btn-revision"><i class="fas fa-edit"></i> Revisi</a>
                                        @elseif($statusVerifikasi == 'pending')
                                            <span class="btn-action btn-waiting"><i class="fas fa-clock"></i> Menunggu</span>
                                        @else
                                            <a href="{{ route('mahasiswa.sertifikasi.show', $item->id) }}" class="btn-action btn-detail"><i class="fas fa-eye"></i> Detail</a>
                                        @endif
                                    </div>
                                </td>
                            </tr>
        ...
        @else
            <div class="empty-state">
                ...
                <p>Mulai dengan menambahkan sertifikasi pertama Anda</p>
                <a href="{{ route('mahasiswa.sertifikasi.create') }}" class="btn-add-new"><i class="fas fa-plus-circle"></i> Tambah Sertifikasi</a>
            </div>
        @endif
    </div>
</div>
