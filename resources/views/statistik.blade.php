...
<body>
    ...
    <section style="position: relative; flex: 1; min-height: calc(100vh - 200px); display: flex; flex-direction: column; align-items: center; justify-content: flex-start; background: linear-gradient(135deg, #0f172a 0%, #1e1b4b 100%); padding: 40px 20px; font-family: 'Poppins', sans-serif; color: #ffffff; overflow: hidden;">
        ...
        <div class="content-wrapper">
            ...
            <script type="application/json" id="chartProdiValues">@json($chartProdiValues ?? [])</script>
            <script type="application/json" id="chartProdiKeys">@json($chartProdiKeys ?? [])</script>
            <script type="application/json" id="chartStatusValues">@json($chartStatusValues ?? [])</script>
            <script type="application/json" id="chartStatusKeys">@json($chartStatusKeys ?? [])</script>
            <script type="application/json" id="chartTrendValues">@json($chartTrendValues ?? [])</script>
            <script type="application/json" id="chartTrendKeys">@json($chartTrendKeys ?? [])</script>
            ...
            <script>
                ...
                // 1. Chart Prodi
                var optionsProdi = {
                    series: [{ name: 'Total SKPI', data: parseJson('chartProdiValues') }],
                    chart: { type: 'bar', height: 300, background: 'transparent', toolbar: { show: false } },
                    theme: { mode: 'dark' },
                    grid: { borderColor: 'rgba(255, 255, 255, 0.1)' },
                    plotOptions: { bar: { borderRadius: 4, horizontal: true } },
                    dataLabels: { enabled: false },
                    xaxis: { categories: parseJson('chartProdiKeys') },
                    colors: ['#3b82f6']
                };
                ...
                // 3. Chart: Trend
                var optionsTrend = {
                    series: [{ name: 'Pengajuan', data: parseJson('chartTrendValues') }],
                    ...
                    xaxis: { categories: parseJson('chartTrendKeys') },
                    colors: ['#8b5cf6'],
                    fill: {
                        type: 'gradient',
