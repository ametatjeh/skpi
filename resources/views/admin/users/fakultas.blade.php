...
@section('content')
    ...
    <div class="content-wrapper">
        ...
        <div class="breadcrumb-nav">
            ...
            <span class="current">
                <i class="fas fa-building"></i> User Fakultas
            </span>
        </div>
        ...
        <div class="header-actions">
            <div class="page-header">
                <h2><i class="fas fa-building"></i> Daftar Operator Fakultas</h2>
                <p style="color: #64748b; font-size: 14px; margin-top: 5px;">
                    Kelola akun operator fakultas
                </p>
            </div>
            ...
            <div class="action-group">
                ...
                {{-- SEARCH --}}
                <form action="{{ route('admin.users.fakultas') }}" method="GET" style="display: flex; gap: 10px;">
        ...
        <div class="table-container">
            @if ($users->count() > 0)
                    ...
                    <tbody>
                        @foreach ($users as $i => $user)
                            <tr>
                                ...
                                <td>
                                    @php
                                        $fakultas = DB::table('fakultas')->where('id', $user->fakultas_id)->first();
                                    @endphp
                                    @if ($fakultas)
                                        <span class="badge badge-success">{{ $fakultas->nama_fakultas }}</span>
                                    @else
                                        <span class="badge badge-success">ID: {{ $user->fakultas_id ?? '-' }}</span>
                                    @endif
                                </td>
                                <td>
                                    <div class="action-buttons" style="justify-content: center;">
                                        ...
                                        <form action="{{ route('admin.users.fakultas.destroy', $user->id) }}"
                            ...
                            <div id="editModal{{ $user->id }}" class="modal">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h3><i class="fas fa-edit"></i> Edit Operator Fakultas</h3>
                                        <a href="#" class="close-modal">&times;</a>
                                    </div>

                                    <form action="{{ route('admin.users.fakultas.update', $user->id) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                                        ...
                                        <div class="form-group">
                                            <label>Fakultas <span style="color: #ef4444;">*</span></label>
                                            <select name="fakultas_id" class="form-control" required>
                                                <option value="">Pilih Fakultas</option>
                                                @php
                                                    $fakultas_list = DB::table('fakultas')
                                                ...
                                                @foreach ($fakultas_list as $fakultas)
                                                    <option value="{{ $fakultas->id }}"
                                                        {{ $user->fakultas_id == $fakultas->id ? 'selected' : '' }}>
                                                        {{ $fakultas->nama_fakultas }} - Dekan: {{ $fakultas->dekan }}
                                                        (Akreditasi: {{ $fakultas->akreditasi }})
                                                    </option>
                                                @endforeach

                                                @if ($user->fakultas_id && !$fakultas_list->pluck('id')->contains($user->fakultas_id))
                                                    <option value="{{ $user->fakultas_id }}" selected>
                                                        Fakultas ID: {{ $user->fakultas_id }} (Data lama)
                                                    </option>
                                                @endif
                                            </select>
                                        </div>
                ...
                <div class="info-count">
                    Total: <strong>{{ $users->total() }}</strong> operator fakultas
                </div>
            ...
            @else
                <div class="empty-state">
                    ...
                    <p>Belum ada operator fakultas terdaftar</p>
                </div>
            @endif
        </div>
    </div>
    ...
    <div id="addModal" class="modal">
        <div class="modal-content">
            <div class="modal-header">
                <h3><i class="fas fa-user-plus"></i> Tambah Operator Fakultas</h3>
                <a href="#" class="close-modal">&times;</a>
            </div>

            {{-- ✅ FIX: Ganti route dari admin.users.store ke admin.users.fakultas.store --}}
            <form action="{{ route('admin.users.fakultas.store') }}" method="POST">
                @csrf
                ...
                <div style="background: #ecfdf5; padding: 15px; border-radius: 10px; margin-bottom: 20px;">
                    <h4 style="font-size: 16px; font-weight: 700; color: #059669; margin: 0 0 15px 0;">
                        <i class="fas fa-building"></i> Pilih Fakultas
                    </h4>

                    <div class="form-group">
                        <label>Fakultas <span style="color: #ef4444;">*</span></label>
                        <select name="fakultas_id" class="form-control" required style="padding: 12px;">
                            <option value="">-- Pilih Fakultas --</option>
                            @php
                        ...
                        <small style="color: #64748b; font-size: 12px; display: block; margin-top: 5px;">
                            Pilih fakultas yang akan dikelola operator ini
                        </small>
                    </div>
                </div>
                ...
                <button type="submit" class="btn btn-primary" style="width: 100%;">
                    <i class="fas fa-save"></i> Simpan Operator Fakultas
                </button>
            </form>
        </div>
    </div>
