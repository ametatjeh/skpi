...
<div class="content-wrapper">
    <div class="header-section">
        <h5 class="page-title">Edit Program Studi</h5>
        <a href="{{ route('admin.total-prodi.index') }}" class="btn btn-secondary">
            <i class="fas fa-arrow-left"></i> Kembali
        </a>
    </div>

    <form action="{{ route('admin.total-prodi.update', $prodi->id) }}" method="POST">
        @csrf
        @method('PUT')
            ...
            <div class="col-half">
                ...
                <select name="akreditasi" class="form-control @error('akreditasi') is-invalid @enderror">
                    ...
                    @foreach(['Unggul', 'Baik Sekali', 'Baik', 'A', 'B', 'C'] as $akr)
                        <option value="{{ $akr }}" {{ old('akreditasi', $prodi->akreditasi) == $akr ? 'selected' : '' }}>{{ $akr }}</option>
                    @endforeach
                </select>
            </div>
        ...
        <div class="row">
            ...
            <div class="col-half">
                ...
                <select name="kk_prodi" class="form-control @error('kk_prodi') is-invalid @enderror">
                    <option value="6" {{ old('kk_prodi', $prodi->kk_prodi) == '6' ? 'selected' : '' }}>Level 6 (S1/D4)</option>
                    <option value="5" {{ old('kk_prodi', $prodi->kk_prodi) == '5' ? 'selected' : '' }}>Level 5 (D3)</option>
                    <option value="7" {{ old('kk_prodi', $prodi->kk_prodi) == '7' ? 'selected' : '' }}>Level 7 (Profesi)</option>
                    <option value="8" {{ old('kk_prodi', $prodi->kk_prodi) == '8' ? 'selected' : '' }}>Level 8 (Magister)</option>
                    <option value="9" {{ old('kk_prodi', $prodi->kk_prodi) == '9' ? 'selected' : '' }}>Level 9 (Doktor)</option>
                </select>
            </div>
            ...
            <div class="col-half">
                ...
                <input type="text" name="bahasa_pengantar" class="form-control" 
                       value="{{ old('bahasa_pengantar', $prodi->bahasa_pengantar) }}">
            </div>
        </div>

        <div style="margin-top: 30px;">
            <button type="submit" class="btn btn-primary">
                <i class="fas fa-save"></i> Update Data Prodi
            </button>
        </div>
    </form>
</div>
