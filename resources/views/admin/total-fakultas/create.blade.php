...
<div class="content-wrapper">
    <div class="header-section">
        <h5 class="page-title">Tambah Fakultas</h5>
        <a href="{{ route('admin.total-fakultas.index') }}" class="btn btn-secondary">
            <i class="fas fa-arrow-left"></i> Kembali
        </a>
    </div>

    <form action="{{ route('admin.total-fakultas.store') }}" method="POST">
        @csrf
        ...
        <div class="form-group">
            ...
            <input type="text" name="nama_fakultas" class="form-control @error('nama_fakultas') is-invalid @enderror" 
                   value="{{ old('nama_fakultas') }}" required placeholder="Contoh: Fakultas Teknik">
            @error('nama_fakultas')
        ...
        <div class="form-group">
            ...
            <input type="text" name="dekan" class="form-control @error('dekan') is-invalid @enderror" 
                   value="{{ old('dekan') }}" placeholder="Nama Dekan">
            @error('dekan')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div style="margin-top: 30px;">
            <button type="submit" class="btn btn-primary">
                <i class="fas fa-save"></i> Simpan Data Fakultas
            </button>
        </div>
    </form>
</div>
